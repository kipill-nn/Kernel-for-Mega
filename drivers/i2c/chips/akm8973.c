...
int akm8973_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    printk(KERN_INFO "%s: Interrupt mode!!\n", __func__);
    ...
    /* Pull up the reset PIN to enable the Compass function */
    ...
    /* set Compass INT to input PIN */
    err = gpio_request(pdata->intr, "akm8973");
    if (err < 0) {
        printk(KERN_ERR "%s: request interrupt gpio failed\n", __func__);
        goto err_request_reset_gpio;
    }
    err = gpio_direction_input(pdata->intr);
    if (err < 0) {
        printk(KERN_ERR
               "%s: gpio direction input [interrupt] failed\n", __func__);
        goto err_set_reset_gpio;
    }

	err = AKECS_PowerDown();
	if (err < 0) {
		printk(KERN_ERR"%s: set power down mode error\n", __func__);
	...
	if (err) {
		printk(KERN_ERR "%s: Unable to register input device: %s\n"
				, __func__, akm->input_dev->name);
		goto exit_input_register_device_failed;
	}

	err = misc_register(&akmd_device);
	if (err) {
		printk(KERN_ERR "%s: akmd_device register failed\n", __func__);
