...
#define I2C_READ_RETRY_TIMES  		100
#define I2C_WRITE_RETRY_TIMES 		100
...
static atomic_t auto_backlight_enabled = ATOMIC_INIT(0);
...
static int microp_i2c_version_check(struct i2c_client *client, uint8_t *data)
{
	...
	buf[0] = 6;
	buf[1] = FUNC_MICROP_PWM;
	i2c_write_block(client, MICROP_I2C_CMD_PIN_CONFIG, buf, 2);

	buf[0] = 6;
	buf[1] = MICROP_I2C_PWM_FREQ;
	buf[2] = MICROP_PIN_PWM_FREQ_HZ_15600;
	i2c_write_block(client, MICROP_I2C_CMD_PIN_PWM, buf, 3);

	buf[0] = 6;
	buf[1] = MICROP_I2C_PWM_MANUAL;
	buf[2] = 0x80;
	i2c_write_block(client, MICROP_I2C_CMD_PIN_PWM, buf, 3);

	buf[0] = 6;
	buf[1] = 0x01;
	return i2c_write_block(client, MICROP_I2C_CMD_PIN_MODE, buf, 2);
}
...
static int microp_i2c_auto_backlight_set_interrupt_mode(struct i2c_client *client, uint8_t enabled)
{
	...
	if(ret != 0)
		dev_err(&client->dev, "%s: setting MICROP_I2C_CMD_MISC1 fail\n", __func__);
...
void get_3_button_key(void)
{
	...
	if(i2c_read_block(client, MICROP_I2C_CMD_ADC_READ_DATA, data, 3) < 0) {
		...
		if(ret < 0)
			...
			dev_info(&client->dev,  "%s: data: 0x%2.2X, ret: 0x%2.2X\n", __func__, adc_value, ret);
			cnf_driver_event("H2W_3button", &ret);
		}
#endif
	}
...
int set_microp_mic_intr(int enable)
{
	...
	data[7] = microp_i2c_is_intr_all(config);
	if (pin_adc < 8)
		data[1] = enable ? (data[1] | (1 << pin_adc)) : (data[1] & ~(1 << pin_adc));
	else if (pin_adc < 16)
		data[2] = enable ? (data[2] | (1 << (pin_adc - 8))) : (data[2] & ~(1 << (pin_adc - 8)));
	else {
		if (pin_adc == 16)
	...
	if (ret < 0)
		...
		if (ret == pdata->pin_config[node_mic].init_value)
			...
			if (!microp_i2c_is_supported(FUNC_MICROP_ADC_INTR,
				...
				schedule_delayed_work(&hpin_work, HPIN_DELAY);
				data[0] = pdata->pin_config[node_adc].intr_pin;
				data[1] = pdata->pin_config[node_adc].adc_pin;
				ret = i2c_write_block(client, MICROP_I2C_CMD_ADC_INTR,
						data, 2);
				if (ret < 0) {
					dev_err(&client->dev,
						"%s: set microp ADC INT error: 0x%X\n",
						__func__, ret);
					return -EIO;
				}
			}
...
static void microp_auto_backlight_function(void)
{
	...
	if(i2c_read_block(client, MICROP_I2C_CMD_READ_PIN, buffer_tmp, 6) < 0)	{
		...
		dev_dbg(&client->dev,"%s: raw: 0x%X, level: %d #\n",
			__func__, (buffer_tmp[4] << 8 | buffer_tmp[5]), buffer_tmp[3]);
...
static void microp_i2c_intr_work_func(struct work_struct *work)
{
	...
	if(microp_i2c_is_supported(FUNC_MICROP_EN_SPI_AUTO_BACKLIGHT, cdata->version))	{
		...
		dev_info(&client->dev,  "%s: get 0x%2.2X\n", __func__, data[0]);
	}
...
void microp_early_suspend(struct early_suspend *h)
{
	...
	if (atomic_read(&auto_backlight_enabled))
...
void microp_late_resume(struct early_suspend *h)
{
	struct i2c_client *client = private_microp_client;
	struct microp_i2c_client_data *cdata;
	struct microp_i2c_platform_data *pdata;
...
static void light_sensor_activate(struct led_classdev *led_cdev)
{
	...
	if(microp_i2c_auto_backlight_set_interrupt_mode(client, 1) < 0) {
		...
		data->done = 1;
		atomic_set(&auto_backlight_enabled, 1);
	}
}
...
static void light_sensor_deactivate(struct led_classdev *led_cdev)
{
	...
	if(microp_i2c_auto_backlight_set_interrupt_mode(client, 0) < 0) {
		...
		data->done = 1;
		atomic_set(&auto_backlight_enabled, 0);
	}
}
...
static void microp_i2c_gate_work_func(struct work_struct *work)
{
	...
	/* for debugging resume time */
	printk(KERN_INFO "[R] turn %s backlight\n", (ldata->mode)?"on":"off");
	mutex_unlock(&ldata->pin_mutex);
