		...
		struct msm_mddi_client_data *client_data)
{
	...
	if (bridge->panel_type == PANEL_TPO)
		...
		eid_cmd(client_data, SRCCTL, prm, 8, 0x00, 0x00, 0x0a, 0x01,
			0x01, 0x1d, 0x00, 0x00);
	if (bridge->panel_type == PANEL_TPO)
	...
	if (bridge->panel_type == PANEL_EID_40pin) {
		...
		eid_cmd(client_data, GAMCTL1, prm, 16, 0x00, 0x00, 0x00, 0x00,
			0x1f, 0x2b, 0x2c, 0x2c, 0x10, 0x10, 0x0f, 0x16,
			0x06, 0x22, 0x22, 0x00);
		eid_cmd(client_data, GAMCTL2, prm, 16, 0x00, 0x00, 0x00, 0x00,
			0x1f, 0x2b, 0x2c, 0x2c, 0x10, 0x10, 0x0f, 0x16,
			0x06, 0x22, 0x22, 0x00);
		/* auto */
		eid_cmd(client_data, BCMODE, prm, 4, 0x02, 0x00, 0x00, 0x00);
		    ...
		    struct msm_mddi_client_data *client_data)
{
	...
	eid_cmd(client_data, DISCTL, prm, 12, 0x14, 0x14, 0x0f, 0x11, 0x11,
		0x11, 0x11, 0x10, 0x04, 0x14, 0x14, 0x00);
	eid_cmd(client_data, PWRCTL, prm, 12, 0x00, 0x01, 0x26, 0x26, 0x0a,
		0x22, 0x6f, 0x6f, 0x00, 0x00, 0x00, 0x00);
	eid_cmd(client_data, VCMCTL, prm, 8, 0x6d, 0x6d, 0x7a, 0x7a, 0x44, 0x00,
		0x00, 0x00);
	eid_cmd(client_data, SRCCTL, prm, 8, 0x02, 0x00, 0x06, 0xf1, 0x41, 0x1f,
		0x00, 0x00);
	...
	eid_cmd(client_data, GAMCTL1, prm, 16, 0x00, 0x20, 0x00, 0x04, 0x3e,
		0x3c, 0x36, 0x34, 0x0e, 0x0e, 0x0e, 0x16, 0x06, 0x02, 0x22,
		0x00);
	eid_cmd(client_data, GAMCTL2, prm, 16, 0x00, 0x20, 0x00, 0x02, 0x38,
		0x34, 0x30, 0x2f, 0x0e, 0x0e, 0x0a, 0x16, 0x06, 0x02, 0x22,
		0x00);
	eid_cmd(client_data, BCMODE, prm, 4, 0x02, 0x00, 0x00, 0x00);
	eid_cmd(client_data, WRCABC, prm, 4, 0x00, 0x00, 0x00, 0x00);
	eid_cmd(client_data, CASET, prm, 4, 0x00, 0x00, 0x01, 0x3f);
...
static int mddi_samsung_probe(struct platform_device *pdev)
{
	...
	if ((bridge_data->panel_type == PANEL_EID_40pin) ||
		...
		panel->vsync_isr = samsung_vsync_interrupt;
	} else {
		err = -EINVAL;
		goto err_panel;
	}
	...
	panel->panel_data.caps = ~MSMFB_CAP_PARTIAL_UPDATES | MSMFB_CAP_CABC;
	...
	if (bridge_data->caps & MSMFB_CAP_CABC) {
		...
		cabc_config.client = panel->client_data;
