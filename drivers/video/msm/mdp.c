...
int mdp_probe(struct platform_device *pdev)
{
	...
	for (n = 0; n < ARRAY_SIZE(mdp_upscale_table); n++)
	{
		...
		/* Workaround for MSM-7225 MDP issues:
		 * Due to hardware defect(the register MDP_PPP_SCALE_COEFF_LSB
		 * ), sometimes there is grid pattern appears while the
		 * scale-up operation is performed.
		 * Solution: after each mdp_writel(), examine that if the value
		 * been write successfully.
		 */
		for(r = 0 ; r < MDP_SCALE_CFG_RETRY ; r++)
		{
			v = mdp_readl(mdp, mdp_upscale_table[n].reg);
			if ( v == mdp_upscale_table[n].val )
				break ;
			printk(KERN_ERR, "failed to configure "
				"mdp up scale coefficient table: "
				"reg=0x%08X, val=0x%08X\n",
				mdp_upscale_table[n].reg,
				mdp_upscale_table[n].val);
			msleep(100);
			mdp_writel(mdp, mdp_upscale_table[n].val,
					mdp_upscale_table[n].reg);
		}
