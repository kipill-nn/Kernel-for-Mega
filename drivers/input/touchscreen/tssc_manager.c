...
static unsigned char touch_check_left_border = 0;
static unsigned char touch_check_right_border = 0;
static unsigned char touch_check_upper_border = 1;
static unsigned char touch_check_bottom_border = 0;
...
static int touch_check_up(int pressure)
{
	...
	#if 1
	if (
	(1 == tssc_reg->tssc_status.penirq_status)
	...
	#else
	if (!tssc_reg->tssc_status.penirq_status || (!tssc_reg->tssc_ctl.intr_flag2 && tssc_reg->tssc_status.busy))
		return 0;
	else
		return 1;
	#endif
...
static void touch_input_up(struct input_dev *dev)
{
	...
	if (touch_report_count > 0) {
		...
		if (debug_tp & STATISTIC_TP_ON) printk(KERN_DEBUG "touch_input_up(): input_sync(): (%d-%02d-%02d %02d:%02d:%02d.%09lu UTC)\n",
			tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
			tm.tm_hour, tm.tm_min, tm.tm_sec, ts.tv_nsec);
		if (debug_tp & STATISTIC_TP_ON) printk(KERN_DEBUG "touch_input_up(): input_sync():\n");
	}
}
...
static ssize_t touch_border_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (debug_tp & DEBUG_TP_ON) printk(KERN_DEBUG "touch_border_store():\n");
	while ((p = strsep(&ptr_data, " "))) {
		...
		para[cnt] = val;
		if (debug_tp & DEBUG_TP_ON) printk(KERN_DEBUG "touch_border_store(): para[%d]=%ld\n", cnt, para[cnt]);
		#endif

		cnt++;
	}
	if (debug_tp & DEBUG_TP_ON) printk(KERN_DEBUG "touch_border_store(): cnt=%d\n", cnt);
	...
	if (cnt == 4) {
		...
		if (debug_tp & DEBUG_TP_ON) printk(KERN_DEBUG "touch_border_store(): %ld %ld %ld %ld\n", para[0], para[1], para[2], para[3]);

		TOUCH_BORDER_X1 = para[0];

		#if 0
		TOUCH_BORDER_X2 = para[1];
		#else
...
static ssize_t touch_border_read(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	if (debug_tp & DEBUG_TP_ON) printk(KERN_DEBUG "%s():\n", __func__);
...
{
	if (debug_tp & DEBUG_TP_ON) printk(KERN_DEBUG "touch_check_border(): x=%d y=%d\n", x , y);

	#if 0
	if (x < TOUCH_BORDER_X1 && touch_check_left_border == 1) {
	#else
	if (x > TOUCH_BORDER_X11 && x < TOUCH_BORDER_X1 && touch_check_left_border == 1) {
	#endif
		if (debug_tp & DEBUG_TP_ON) printk(KERN_DEBUG "x is in left border\n");
		...
		#if 0
		input_report_abs(dev, ABS_X, TOUCH_BORDER_X11);
		input_report_abs(dev, ABS_Y, y);
		touch_check_border_report(dev);
		#endif
	}
	...
	#if 0
	if (x > TOUCH_BORDER_X2 && touch_check_right_border == 1) {
	#else
	if (x > TOUCH_BORDER_X21 && x < TOUCH_BORDER_X2 && touch_check_right_border == 1) {
	#endif
		if (debug_tp & DEBUG_TP_ON) printk(KERN_DEBUG "x is in right border\n");
		input_report_abs(dev, ABS_X, TOUCH_BORDER_X20);
		input_report_abs(dev, ABS_Y, y);
		touch_check_border_report(dev);

		#if 0
		input_report_abs(dev, ABS_X, TOUCH_BORDER_X21);
		input_report_abs(dev, ABS_Y, y);
		touch_check_border_report(dev);
		#endif
	}
	...
	#if 0
	if (y < TOUCH_BORDER_Y1 && touch_check_upper_border == 1) {
	#else
	if (y > TOUCH_BORDER_Y11 && y < TOUCH_BORDER_Y1 && touch_check_upper_border == 1) {
	#endif
		if (debug_tp & DEBUG_TP_ON) printk(KERN_DEBUG "y is in upper border\n");
		...
		#if 0
		input_report_abs(dev, ABS_X, x);
		input_report_abs(dev, ABS_Y, TOUCH_BORDER_Y11);
		touch_check_border_report(dev);
		#endif
	}
	#if 0
	if (y > TOUCH_BORDER_Y2 && touch_check_bottom_border == 1) {
	#else
	if (y > TOUCH_BORDER_Y21 && y < TOUCH_BORDER_Y2 && touch_check_bottom_border == 1) {
	#endif
		if (debug_tp & DEBUG_TP_ON) printk(KERN_DEBUG "y is in bottom border\n");
		input_report_abs(dev, ABS_X, x);
		...
		#if 0
		input_report_abs(dev, ABS_X, x);
		input_report_abs(dev, ABS_Y, TOUCH_BORDER_Y21);
		touch_check_border_report(dev);
		#endif
	}
}
...
static void touch_process_queue(struct input_dev *dev)
{
	...
	if (debug_tp & DEBUG_TP_ON) printk(KERN_DEBUG "touch_process_queue(): touch_sample_count= %ld\n", touch_sample_count);
	if (touch_sample_count <= 1) {
		if (touch_sample_count == 1) {
			if (debug_tp & DEBUG_TP_ON) printk(KERN_DEBUG "This is point #%ld.\t", touch_sample_count);
			if (debug_tp & STATISTIC_TP_ON)printk(KERN_DEBUG "touch_process_queue(): tssc_reg->tssc_ctl.intr_flag2=0x%x\t", tssc_reg->tssc_ctl.intr_flag2);
			if (debug_tp & DEBUG_TP_ON) {
				getnstimeofday(&ts);
				rtc_time_to_tm(ts.tv_sec, &tm);
			}
	...
	if (x >= 0 && y >= 0) {
		if (debug_tp & DEBUG_TP_ON) printk(KERN_DEBUG "touch_process_queue():\t");
		...
		if (debug_tp & DEBUG_TP_ON) printk(KERN_DEBUG "x=%d\t", x);
		input_report_abs(dev, ABS_X, x);
...
static irqreturn_t tssc_manager_irq_down_handler(int irq, void *dev_id)
{
	...
	if (debug_tp & STATISTIC_TP_ON) printk(KERN_DEBUG "%s(): enter\n", __func__);

	disable_irq(msm7225_irq_down);
	schedule_work(&ts->work);

	if (debug_tp & STATISTIC_TP_ON) printk(KERN_DEBUG "%s(): exit\n", __func__);
...
static void tssc_power_on(void)
{
	...
	#if 0
	printk(KERN_DEBUG "tssc_power_on(): tssc_codec=%p\n", tssc_codec);
	printk(KERN_DEBUG "tssc_power_on(): tssc_codec->ctl=0x%lx\n", tssc_codec->ctl);
	printk(KERN_DEBUG "tssc_power_on(): tssc_codec->priorities.priority0=0x%x\n", tssc_codec->priorities.priority0);
	#endif
	...
	#if 0
	printk(KERN_DEBUG "tssc_power_on(): tssc_reg=%p\n", tssc_reg);
	printk(KERN_DEBUG "tssc_power_on(): tssc_reg->tssc_ctl.enable=0x%x\n", tssc_reg->tssc_ctl.enable);
	#endif
