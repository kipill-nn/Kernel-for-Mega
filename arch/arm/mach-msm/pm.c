 ...
 * During booting up, disable entering idle power saving mode.
 * Re-enable it after booting up BOOT_LOCK_TIMEOUT sec.
 */
#define BOOT_LOCK_TIMEOUT	(80 * HZ)
static int boot_idle_lock = 1;
static void do_expire_boot_lock(struct work_struct *work)
{
	boot_idle_lock = 0;
	pr_info("Release 'boot-time' idle_lock\n");
}
...
static int msm_sleep(int sleep_mode, uint32_t sleep_delay, int from_idle)
{
	...
	if (sleep_mode < MSM_PM_SLEEP_MODE_APPS_SLEEP) {
		...
		if (!from_idle)
			printk(KERN_INFO "[R] resume starts collpased=%d\n",
						collapsed);
                               ...
                               int count, int *eof, void *data)
{
	...
	for (i = 0; i < ARRAY_SIZE(msm_pm_stats); i++) {
		...
		p += snprintf(p, end - p, "  >=%2lld.%09u: %7d (%lld-%lld)\n",
			s, ns, msm_pm_stats[i].bucket[j],
			msm_pm_stats[i].min_time[j],
			msm_pm_stats[i].max_time[j]);
	}
...
static int __init msm_pm_init(void)
{
	...
	pr_info("Acquire 'boot-time' idle_lock\n");
	return 0;
}
