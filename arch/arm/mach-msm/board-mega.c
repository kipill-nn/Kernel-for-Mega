...
{
        printk("\n\nboard-mega.c: banamas reset\n\n");
	gpio_set_value(BAHAMAS_GPIO_PS_HOLD, 0);
}

static uint32_t mega_serial_debug_table[] = {
...
static void config_gpio_table(uint32_t *table, int len)
{
	int n;
	unsigned id;
//        printk("\n\nboard-mega.c: config gpio table, len = %d\n\n", len);
	for(n = 0; n < len; n++) {
		id = table[n];
...
void config_bahamas_camera_on_gpios(void)
{
//        printk("\n\nboard-mega.c: config bahamas camera on gpios\n\n");
...
static void mega_config_serial_debug_gpios(void)
{
	config_gpio_table(mega_serial_debug_table,
			ARRAY_SIZE(mega_serial_debug_table));
        printk("\n\nconfig gpio table\n\n");
}
...
//        printk("\n\nboard-mega.c: config gpios\n\n");
	mega_config_serial_debug_gpios();
//        printk("\n\nboard-mega.c: mega config serial debug gpios\n\n");
